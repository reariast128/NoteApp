...
namespace NoteApp
{
    class Tui
    {
        ...
        {
            if (_esC.IsMatch(opcion) || _esQ.IsMatch(opcion) ||
                (_esNumero.IsMatch(opcion) && int.Parse(opcion) <= _controlador.ObtenerTitulosCuaderno().Count) ||
                _esM.IsMatch(opcion) || _esT.IsMatch(opcion))
            {
                return true;
            }
            else
            ...
            try
            {
                ...
                var cuaderno = _controlador.SeleccionarCuaderno(idCuaderno);
                {
                    while (true)
                    {
                        ...
                        Console.WriteLine($"==================================================  \n\n");
                        Console.WriteLine(
                            "Selecciona el id de la nota a ver, o 't' para modificar el título del cuaderno, o 'c' para crear una nueva nota, o 'q' para volver a la selección de cuadernos.");
        ...
        private void MostrarNota(string idNota, int idCuaderno)
        {
            try
            {
                ...
                Console.WriteLine(
                    "Introduce 'q' para volver al selector de notas, 'm' para modificar el contenido, o 't' para modificar el título.");
